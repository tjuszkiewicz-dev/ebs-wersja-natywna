...
- **Wystawianie (przepływ 2-etapowy)**: `PATCH /api/orders/[id]/hr-confirm` po utworzeniu lokalnych `financial_documents` woła `issueDocumentsForOrder(..., 'nota')` — wystawia w FA **tylko notę księgową**. **Faktura VAT jest odroczona** i wystawia się dopiero **po oznaczeniu noty jako opłaconej** (webhook/cron → `issueFakturaForOrder`), z auto-KSeF po stronie konta FA. Failure-tolerant — awaria FA nie blokuje potwierdzenia. Prowizja na fakturze VAT = `companies.fee_percent` (migracja 015, `NOT NULL DEFAULT 20`, zakres 15–31%; docelowo 10–32 — patrz spec CRM); fallback 20% gdy brak.
- **Sync płatności**: webhook `POST /api/webhooks/fakturownia?secret=...` (instant) + cron `GET /api/cron/sync-fakturownia-payments` raz dziennie 06:00 (`vercel.json`, chroniony `CRON_SECRET`; plan Vercel Hobby dopuszcza crony max raz/dobę — webhook jest głównym kanałem, cron to dzienny fallback-reconcyliacja). Webhook nie ufa polu `status` z payloadu — po sprawdzeniu sekretu potwierdza stan faktury bezpośrednio w FA (`fa.getInvoice`) i dopiero `status==='paid'` oznacza dokument jako opłacony. **Po opłaceniu NOTY oba kanały wołają `issueFakturaForOrder`** → wystawienie faktury VAT w FA.
