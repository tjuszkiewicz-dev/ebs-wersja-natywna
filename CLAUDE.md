...
- **Moduły** `lib/fakturownia/`:
  ...
  - `invoiceService.ts` — `ensureClient` (mapuje firmę EBS → klienta FA po NIP, cache w `companies.fakturownia_client_id`), `buildNotaInput`/`buildFakturaInput`, `issueDocumentsForOrder` (idempotentne, zwraca `IssueResult {issued,failed,skipped}`, zapisuje `fakturownia_invoice_id`/`token`/`payment_url`/`pdf_url`/`fakturownia_sync_status` do `financial_documents`). Ochrona przed duplikatem: błąd `createInvoice` → `sync_status='failed'` (bezpieczny retry); ale jeśli faktura powstała w FA, a zapis id do DB padnie, ponawia zapis (3×) i NIE oznacza `failed` — loguje `KRYTYCZNE` do ręcznej reconcyliacji.
  - `factory.ts` — `getFakturowniaClient()` z env.
- **Wystawianie**: `PATCH /api/orders/[id]/hr-confirm` po utworzeniu lokalnych `financial_documents` woła `issueDocumentsForOrder` (failure-tolerant — awaria FA nie blokuje potwierdzenia; dokument zostaje z `sync_status='failed'`). Prowizja na fakturze VAT = `companies.fee_percent` (migracja 015, `NOT NULL DEFAULT 20`, zakres 15–31%) — ta sama wartość, którą widzą dokumenty lokalne; fallback 20% gdy brak.
- **Sync płatności**: webhook `POST /api/webhooks/fakturownia?secret=...` (instant) + cron `GET /api/cron/sync-fakturownia-payments` raz dziennie 06:00 (`vercel.json`, chroniony `CRON_SECRET`; plan Vercel Hobby dopuszcza crony max raz/dobę — webhook jest głównym kanałem, cron to dzienny fallback-reconcyliacja). Webhook nie ufa polu `status` z payloadu — po sprawdzeniu sekretu potwierdza stan faktury bezpośrednio w FA (`fa.getInvoice`) i dopiero `status==='paid'` oznacza dokument jako opłacony.
