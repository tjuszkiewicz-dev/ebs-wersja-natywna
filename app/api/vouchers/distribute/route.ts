...
// HR → pracownik przez distribute_to_employee() — przesuwa fizyczne rekordy + accounting.
...
export async function POST(req: NextRequest) {
  ...
  if (!companyId) {
    return NextResponse.json({ error: 'Brak powiązania HR z firmą.' }, { status: 400 });
  }
  ...
  // Przesuń fizyczne rekordy voucherów do pracownika (+ accounting atomicznie)
  const { data: distributed, error: distErr } = await (supabase.rpc as any)('distribute_to_employee', {
  ...
  // Pobierz dane pracownika do protokołu
  ...
  await supabase.from('distribution_batches').insert({
    ...
    total_amount: distributed ?? amount,
    status:       'completed',
  });
  ...
  await supabase.from('notifications').insert({
    user_id: employeeId,
    message: `Otrzymałeś ${distributed ?? amount} nowych voucherów!`,
    type:    'SUCCESS',
  });

  return NextResponse.json({ distributed: distributed ?? amount, batchId });
}
