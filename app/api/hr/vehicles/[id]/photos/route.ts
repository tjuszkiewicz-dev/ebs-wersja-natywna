// GET  /api/hr/vehicles/[id]/photos — zdjęcia stanu pojazdu (signed URLs)
// POST — multipart: file + caption? (dokumentacja przed wydaniem do dyspozycji)
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await sb.from('hr_vehicle_photos').select('*').eq('vehicle_id', id).order('created_at', { ascending: false });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  const photos = await Promise.all((data ?? []).map(async (p: any) => {
    const { data: s } = await sb.storage.from('vehicle-photos').createSignedUrl(p.path, 3600);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const sb = admin() as any;
  const { data: v } = await sb.from('hr_vehicles').select('make, registration').eq('id', id).single();
  ...
  // każdy format graficzny; HEIC/HEIF (iPhone) → JPEG
  let img;
  ...
  const up = await sb.storage.from('vehicle-photos').upload(path, img.buf, { contentType: img.contentType });
  if (up.error) return NextResponse.json({ error: up.error.message }, { status: 500 });

  const { data: row, error } = await sb.from('hr_vehicle_photos')
    .insert({ vehicle_id: id, path, filename: safe, caption, uploaded_by: isUuid(String(auth.id)) ? auth.id : null })
    .select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const sb = admin() as any;
  const { data: photo } = await sb.from('hr_vehicle_photos').select('*').eq('id', photoId).eq('vehicle_id', id).single();
  ...
  await sb.storage.from('vehicle-photos').remove([photo.path]);
  const { error } = await sb.from('hr_vehicle_photos').delete().eq('id', photoId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
